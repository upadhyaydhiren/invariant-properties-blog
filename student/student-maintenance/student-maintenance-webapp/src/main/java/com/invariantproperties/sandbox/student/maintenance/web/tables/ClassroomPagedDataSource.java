...
public class ClassroomPagedDataSource implements GridDataSource {

    private int startIndex;
    private List<Classroom> preparedResults;

    private final ClassroomFinderService classroomFinderService;

    public ClassroomPagedDataSource(ClassroomFinderService classroomFinderService) {
        this.classroomFinderService = classroomFinderService;
    }

    @Override
    public int getAvailableRows() {
        long count = classroomFinderService.count();
        return (int) count;
    }
    ...
    public void prepare(final int startIndex, final int endIndex, final List<SortConstraint> sortConstraints) {
        ...
        // Get a page of classrooms - ask business service to find them (from
        // the
        // database)
        // List<SortCriterion> sortCriteria = toSortCriteria(sortConstraints);
        // preparedResults = classroomFinderService.findClassrooms(startIndex,
